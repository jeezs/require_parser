def resolve_requires(file_path, root_dir, processed_files = Set.new, depth = 0)
  ...
  content.gsub!(/^(require|require_relative)\s+['"](.*?)['"]$/) do |match|
    ...
    required_file = if type == "require_relative"
                      ...
                      File.join(root_dir, required_file_name + ".rb")
                    end

    if File.exist?(required_file)
      resolve_requires(required_file, root_dir, processed_files, depth + 1)
    else
      match
    end
  end
...
def generate_resolved_file(source_file_path, output_file_path)
  root_dir = File.dirname(File.expand_path(source_file_path))
  resolved_content = resolve_requires(source_file_path, root_dir)
  puts(resolved_content)
  File.write(output_file_path, resolved_content)
end

# Utilisation:
source_file_path = "rbtest/index.rb"
